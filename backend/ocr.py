...
class DBPostProcess(object):
    ...
    def boxes_from_bitmap(self, pred, _bitmap, dest_width, dest_height):
        ...
        for index in range(num_contours):
            ...
            scores.append(score)
        return np.array(boxes, dtype=int), scores
...
class det_rec_functions(object):
    ...
    def __init__(self, image, det_file, rec_file, ocr_keys_file, use_large=False):
        self.img = image.copy()
        self.det_file = det_file
        self.small_rec_file = rec_file
        self.onet_det_session = onnxruntime.InferenceSession(self.det_file)
        if use_large:
            print("can not use large model")
            exit()
        else:
        ...
        self.postprocess_op = process_pred(ocr_keys_file, 'ch', True)
...
def filter_box_rec(dt_boxes, rec_results, drop_score=0.5):
...
# class DrawImage():
#
#     def __init__(self, image, boxes, txt, scires):

def draw_ocr_box_txt(image, boxes,  txts=None, scores=None, drop_score=0.5, font_path="simfang.ttf"):
...
def find_text_by_ocr(image_path, ocr_text):
    font_path = "ocr_config/simfang.ttf" # 画图显示的字体
    det_file = 'ocr_config/det.onnx' # 字体检测模型
    rec_file = 'ocr_config/rec.onnx' # 字体识别模型
    ocr_keys_file = r"ocr_config/ppocr_keys_v1.txt" #字体映射文件
    result = None

    image = cv2.imread(image_path)
    # OCR-检测-识别
    ocr_sys = det_rec_functions(image, det_file, rec_file, ocr_keys_file)
    # 得到检测框
    dt_boxes = ocr_sys.get_boxes()
    # 识别 results: 单纯的识别结果，results_info: 识别结果+置信度
    rec_results, rec_results_info = ocr_sys.recognition_img(dt_boxes)
    # 根据置信度对字体框进行过滤
    dt_boxes, rec_results = filter_box_rec(dt_boxes, rec_results)
    for res in rec_results:
        if ocr_text in res[0]:
            result = res[0]
        # print(res[0], res[1])
        pass
    # 转换成ImageArray格式
    ...
    boxes = dt_boxes
    # 提取出识别的文本和对应的分数
    txts = [rec_results[i][0] for i in range(len(rec_results))]
    scores = [rec_results[i][1] for i in range(len(rec_results))]
    ...
    # # 画出识别图
    # draw_img = draw_ocr_box_txt(image, boxes, txts, scores, drop_score=0.5, font_path=font_path)
    # cv2.imshow("test", draw_img[:, :, ::-1])
    # cv2.waitKey(0)


if __name__=='__main__':
    find_text_by_ocr("/Users/ryu/MyProjects/Scripts/wangchao/backend/images/gather_panel.jpg", "集结时间")
